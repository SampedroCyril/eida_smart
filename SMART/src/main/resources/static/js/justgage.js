...
JustGage = function(config) {
    ...
    obj.config = {
        ...
        noGreidant: kvLookup('noGreidant', config, dataset, false),
    ...
    obj.level = obj.canvas.path().attr({
        "stroke": "none",
        "fill": getColor(obj.config.value, (obj.config.value - obj.config.min) / (obj.config.max - obj.config.min), obj.config.levelColors, obj.config.noGreidant, obj.config.customSectors),
        pki: [
            obj.config.min,
            obj.config.min,
            obj.config.max,
...
JustGage.prototype.refresh = function(val, max) {
    ...
    color = getColor(val, (val - obj.config.min) / (obj.config.max - obj.config.min), obj.config.levelColors, obj.config.noGreidant, obj.config.customSectors);
...
/** Get color for value */
function getColor(val, pct, col, noGreidant, custSec) {

    var no, inc, colors, percentage, rval, gval, bval, lower, upper, range, rangePct, pctLower, pctUpper, color;
    var noGreidant = noGreidant || custSec.length > 0;
    ...
    inc = (noGreidant) ? (1 / no) : (1 / (no - 1));
    colors = [];
    for (i = 0; i < col.length; i++) {
        percentage = (noGreidant) ? (inc * (i + 1)) : (inc * i);
